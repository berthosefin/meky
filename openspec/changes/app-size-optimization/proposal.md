...
- Ajouter la dépendance de build `expo-build-properties` et configurer `android.abiFilters` dans `app.json` pour ne conserver que les ABIs utilisées par des téléphones réels : `arm64-v8a` et `armeabi-v7a` (compat 32-bit conservée).
...
- `app.json` : plugin `expo-build-properties` avec `android.abiFilters`.
